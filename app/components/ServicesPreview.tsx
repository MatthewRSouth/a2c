  ...
  return (
    <section className="bg-cream py-20 lg:py-28">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-12">
        <p className="text-[11px] tracking-[0.18em] text-ember mb-4">— SERVICES —</p>
        <h2 className="text-[36px] font-normal text-ink mb-12">サービス</h2>
          ...
          {services.map((s) => (
            ...
            >
              <div className="flex-1 p-6 lg:p-8 flex flex-col min-w-0">
                <p className="text-[10px] tracking-widest text-mute mb-3">
                  {s.num} / 04 — {s.en}
                </p>
                <h3 className="text-[22px] font-medium text-ink mb-3">{s.name}</h3>
                <p className="text-[13px] text-ink-soft leading-relaxed mb-4 flex-1">
                  {s.desc}
                </p>
                <ul className="grid grid-cols-2 gap-y-1.5 mb-5">
                  {s.bullets.map((b) => (
                    <li key={b} className="flex items-center gap-1.5 text-[12px] text-ink-soft">
                      <span className="w-1 h-1 rounded-full bg-ember shrink-0" />
                      {b}
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  className="text-[12px] text-ink hover:text-ember transition-colors tracking-wide"
                >
                  — {s.en}
                </Link>
              </div>
